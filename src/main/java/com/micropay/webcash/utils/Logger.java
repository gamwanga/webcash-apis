...
public class Logger {
    ...
    private static String realm = "glyde", logsPath = "Glyde" + File.separator + "logs";
    ...
    static void purgeOldLogs()
    {
        ...
        for (File log : logs.listFiles())
        {
            ...
            if (Calendar.getInstance().get(Calendar.MONTH) - c1.get(Calendar.MONTH) >= 0 && Calendar.getInstance().get(Calendar.YEAR) - c1.get(Calendar.YEAR) >= 1)
            {
                try
                {
